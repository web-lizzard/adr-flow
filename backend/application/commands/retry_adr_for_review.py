...
class RetryAdrForReviewCommand:
    adr_id: UUID
    user_id: UUID


@dataclass(frozen=True, slots=True)
class RetryAdrForReviewResult:
    stored_event: StoredEvent


class RetryAdrForReviewCommandHandler:
    def __init__(self, uow_factory: UnitOfWorkFactory) -> None:
        self._uow_factory = uow_factory
        self._logger = get_logger(__name__)

    async def handle(
        self, command: RetryAdrForReviewCommand
    ) -> RetryAdrForReviewResult:
        adr_id = str(command.adr_id)
        user_id = str(command.user_id)
        self._logger.info(
            "command.retry_adr_for_review.started",
            adr_id=adr_id,
            user_id=user_id,
        )
        ...
        async with self._uow_factory.begin() as uow:
            ...
            if adr is None or adr.user_id.value != command.user_id:
                self._logger.info(
                    "command.retry_adr_for_review.rejected",
            ...
            new_adr = adr.retry_review(updated_at)
            ...
            self._logger.info(
                "command.retry_adr_for_review.event_appended",
                adr_id=adr_id,
                stored_event_id=stored_event_id,
            )
            self._logger.info(
                "command.retry_adr_for_review.completed",
                adr_id=adr_id,
                stored_event_id=stored_event_id,
            )
            return RetryAdrForReviewResult(stored_event=stored_event)
