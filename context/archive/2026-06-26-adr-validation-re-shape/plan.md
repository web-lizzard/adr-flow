...
- [x] 3.1 `test_review_quality.py` updated for rating schema validation — dfb8816
- [x] 3.2 Handler tests pass with strict failure behavior — dfb8816
- [x] 3.3 `cd backend && uv run pytest tests/application/ -q` — dfb8816
...
- [x] 4.1 API integration test asserts section_ratings in GET response — cdffc21
- [x] 4.2 `cd frontend && pnpm run typecheck && pnpm run lint` — cdffc21
- [x] 4.3 `cd frontend && pnpm run test -- adr-review-annotations adr-editor-page` — cdffc21

#### Manual

- [x] 4.4 Browser: ratings visible below annotations; score-0 gaps display correctly — cdffc21

### Phase 5: Tests, Docs & S-07 Close-Out

#### Automated

- [x] 5.1 `cd backend && uv run pytest tests/review_quality/ -q` — 1e3473c
- [x] 5.2 `cd backend && uv run pytest` — 1e3473c
- [x] 5.3 `pre-commit run --all-files` on touched files — 1e3473c

#### Manual

- [x] 5.4 PRD and roadmap accurately reflect superseded S-07 and new validation model — 1e3473c
